# Hono Drizzle Schema 对齐规范

## 目标
定义 `packages/db` 中 Drizzle schema 与 `infra/docker/postgres/init/001_init_schema.sql` 的一致性要求，并约束 `apps/api` 通过统一数据库包消费表定义、关系定义与客户端。

## 需求
### 需求：Drizzle schema 必须完整映射初始化数据库表
...
### 需求：Drizzle schema 必须保留关键列约束与默认值
